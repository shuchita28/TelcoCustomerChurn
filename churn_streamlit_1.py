...
percent = 0
df_model2 = pd.read_csv('/Users/shuchitamishra/Desktop/DS5220 SML/Project/FinalProcessed.csv')
...
def predict_new(model_name, model, X_train, y_train, X_test, y_test):
	...
	with col1:
		senior_citizen = st.radio('Are they a Senior citizen?', ['Yes', 'No'])
	with col2:
		partner = st.radio('Do they have a partner?', ['Yes', 'No'])
	with col3:
		dependent = st.radio('Do they have dependents?', ['Yes', 'No'])
	with col4:
		phone_services = st.radio('Have they subscibed to phone services?', ['Yes', 'No'])
	with col5:
		internet_services = st.selectbox('Which type of internet services have they subscribed to?', ['DSL', 'Fiber optic', 'No'])
	col11, col12, col13, col14, col15 = st.columns(5)
	with col11:
		online_security = st.selectbox('Have they subscibed to online security?', ['Yes', 'No', 'No internet services'])
	with col12:
		online_backup = st.selectbox('Have they subscibed to online backup?', ['Yes', 'No', 'No internet services'])
	with col13:
		device_protection = st.selectbox('Have they subscibed to device protection?', ['Yes', 'No', 'No internet services'])
	with col14:
		tech_support = st.selectbox('Have they subscibed to tech support?', ['Yes', 'No', 'No internet services'])
	with col15:
		contract = st.selectbox('What kind of contract is established', ['Month-to-month', 'Two Year', 'One Year'])
	col21, col22 = st.columns(2)
	with col21:
		paperless_billing = st.radio('Do they have paperless billing', ['Yes', 'No'])
	with col22:
	...
	with col24:
		monthly_payment = st.slider("Enter monthly payment", 15, 200)
	with col25:
		Total_payment = st.slider("Enter total amount paid", 15, 9000)
	if st.button('Predict Now'):
       ...
       'Contract','Paperless Billing','Payment Method'])
		...
		#print(pred)
		st.write("The probability of customer not churning is ",pred[0][0])
...
def run(X_train,y_train, X_test, y_test,d):
	...
	model = st.selectbox(
	     ...
	     ['', 'Ensemble (issues)', 'Logistic Regression', 'SVM', 'Random Forest', 'KNN','XGBoost(issues)','Lightboost(issues)','Neural Network'])
